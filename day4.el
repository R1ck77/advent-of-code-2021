...
(defun day4/part-1 (blocks)
  (let ((problem (day4/read-problem blocks)))
    (let ((indexed-solution (car (--sort (< (elt it 0)
                                            (elt other 0))
                                         (day4/index-blocks problem)))))
      (* (elt indexed-solution 1) (day4/sum-remaining-values (elt indexed-solution 2))))))
